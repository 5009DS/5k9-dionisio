...
/* ═══════════════════════════════════════════════════════════════════════════
   TOPNAV — a mesma barra do 5K9 Forms e do 5K9 Gestor, com os destinos do
   Dionísio.

   Herda a gramática do design system: marca à esquerda, links ao centro com o
   ativo sublinhado por um gradiente que corre devagar, ações circulares à
   direita. Quem usa mais de um sistema não precisa reaprender nada.

   O editor não é destino de navegação — é onde se chega abrindo um roteiro,
   por isso não tem link próprio aqui. Configurações vive no menu do avatar,
   como no Gestor.

   O selo de MODO LOCAL é o mesmo do Gestor, e importa pelo mesmo motivo: em
   modo local o roteiro é só deste navegador, e escrever a tarde inteira
   achando que o time está vendo é um trabalho que evapora.
   ═══════════════════════════════════════════════════════════════════════════ */

const NAV_ITEMS = [
    { href: '/',          label: 'Roteiros',  match: (p) => p === '/' || p.startsWith('/roteiro') },
...
export const renderTopnav = (container, caminhoAtual) => {
    ...
    container.innerHTML = `
        <header class="tn no-print">
            <div class="tn__brand">
                <a href="/" class="tn__marca" aria-label="5K9 Dionísio — início">
            ...
            <div class="tn__actions">
                ${store.modo === 'local' ? `
                    <a href="/configuracoes" class="tn__modo" title="Os roteiros estão salvos apenas neste navegador. Clique para conectar um banco.">
...
function injectStyles() {
    ...
    style.textContent = `
        ...
        /* ── Marca ──────────────────────────────────────────────────────────
           Dois elementos: o logo, que leva ao início, e o trocador de
           ferramenta ao lado. Um <a> englobando os dois faria um botão
           aninhado num link, e o clique disputaria entre navegar e abrir. */
        ...
        /* ── Responsivo ────────────────────────────────────────────────────
           Com um destino só, o menu central não precisa da linha própria que
           o Gestor usa: cabe ao lado da marca em qualquer largura. */
        @media (max-width: 900px) {
            .tn { gap: var(--space-3); }
            .tn__nav { justify-content: flex-end; }
        }
    `;
